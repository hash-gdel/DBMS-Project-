...
def low_stock():
    print("\n=== MINI-TASK 3: Low Stock Report (Per Hospital) ===")
    ...
    try:
        with get_connection() as cnx:
            with cnx.cursor(dictionary=True) as cur:
                ...
                print(f"{'Hospital':<20} | {'Medication':<20} | {'Qty':<5} | {'Level':<5} | {'Status'}")
                print("-" * 75)
                for row in results:
                    print(f"{row['HospitalName'][:20]:<20} | {row['MedicationName'][:20]:<20} | {row['Qty']:<5} | {row['ReorderLevel']:<5} | {row['Status']}")
                return results
    except Exception:
        cnx.rollback()
        raise
